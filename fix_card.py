...
export default function CharacterCard({ character, isSelected, onClick }: Props) {
  ...
  const handleGenerateAvatar = (e: React.MouseEvent) => {
    ...
    const prompt = encodeURIComponent('Portrait of ' + character.name + '. ' + character.appearance + '. Painterly illustration, soft lighting, detailed face.')
    const url = 'https://image.pollinations.ai/prompt/' + prompt + '?width=512&height=512&nologo=true&model=flux&seed=' + Date.now()
    updateCharacterAvatar(character.id, url)
  }
    ...
    <div onClick={onClick} className={'group relative bg-white dark:bg-gray-900 rounded-2xl border cursor-pointer transition-all duration-200 overflow-hidden ' + cls}>
      <div className="relative aspect-square overflow-hidden bg-gray-50 dark:bg-gray-800">
        {character.avatar_url ? (
          <Image src={character.avatar_url} alt={character.name} fill className="object-cover" sizes="200px" unoptimized />
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3" style={{ background: character.color + '15' }}>
            ...
            <button onClick={handleGenerateAvatar} className="text-xs px-3 py-1.5 rounded-full border opacity-0 group-hover:opacity-100 bg-white dark:bg-gray-900 border-gray-200 text-gray-600 hover:text-gray-900 transition-all">
              Нарисовать
            </button>
          </div>
        )}
...
with open('src/components/characters/CharacterCard.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print('done')
